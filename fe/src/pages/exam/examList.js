...
class  ExamList  extends Component {
  ...
  onAddExam(){
    const flag = '-1'
    this.props.history.push('/admin/exam/new/' + flag)
  }

  //跳转到成绩管理页面
  onNavToGrade(record){
    const exam = []
    exam.push(record.key,record.examTitle, record.examStartTime, record.examEndTime)
...
//formatDate(new Date(((new Date(item.examStartTime)).getTime()) + item.examTotalTime * 60 * 1000), 'yyyy-MM-dd hh:mm')
const mapStateToProps = (state) => {
  ...
  let tableList = newList.map((item) => {
    return {
      ...
      examStartTime: formatDate(new Date(item.timeStart), 'yyyy-MM-dd hh:mm'),
      examEndTime: formatDate(new Date(item.timeEnd), 'yyyy-MM-dd hh:mm')
    }
  })
  return {
